...
const defaultConfig: JssConfig = {
  ...
  maxmindAccountId: process.env[`${constantCase('maxmindAccountId')}`],
  maxmindLicenseKey: process.env[`${constantCase('maxmindLicenseKey')}`],
  corsDomains:
    process.env[`${constantCase('corsDomains')}`] ||
    process.env[`${constantCase('sitecoreApiHost')}`],
};
